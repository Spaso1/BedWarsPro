...
public class BPSuivialCommand implements CommandExecutor {
    ...
    private void openEnchantGUI(Player player) {
        ...
        // 锋利 X
        addEnchantBook(gui, 10, "锋利", "DAMAGE_ALL", 10, 5000);

        // 保护 X
        addEnchantBook(gui, 12, "保护", "PROTECTION_ENVIRONMENTAL", 10, 4500);

        // 力量 X
        addEnchantBook(gui, 14, "力量", "ARROW_DAMAGE", 10, 4500);

        // 火焰附加 X
        addEnchantBook(gui, 16, "火焰附加", "FIRE_ASPECT", 10, 4000);

        // 荆棘 X
        addEnchantBook(gui, 28, "荆棘", "THORNS", 10, 6000);

        // 耐久 X
        addEnchantBook(gui, 30, "耐久", "DURABILITY", 10, 3500);
